...
- si le traitement se termine par `print(n=nb_lignes)`: le calcul peut être exécuté partiellement (sauf dans les cas des fonctions d’agrégations), et seules les `nb_lignes` demandées sont affichées. Cela permet de minimiser les ressources et la mémoire utilisées.
...
`DuckDB` étant un moteur SQL à part entière, on peut interagir avec `DuckDB` directement avec des requêtes SQL. Par exemple, en reprenant une table enregistrée précédemment avec la fonction `duckdb::duckdb_register` :

```{r}
DBI::dbGetQuery(conn_ddb, "SELECT * FROM bpe_ens_2018_duckdb") |> head()
```

Vous pouvez bien sûr lire des fichiers `Parquet` ou `CSV` en utilisant les fonctions de duckdb :
...
Et vous pouvez également créez une vue et utiliser par la suite `dbplr::tbl` et `arrow::write_dataset` :

```{r, eval=FALSE}
dbExecute(conn_ddb, "CREATE OR REPLACE VIEW output AS ...")
