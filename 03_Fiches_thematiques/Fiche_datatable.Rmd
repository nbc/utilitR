...
La manipulation de données avec `data.table` est structurée autour de trois principes structurants, pas simple à comprendre lorsqu'on découvre ce *package*:

-   l'opérateur `[...]`;
-   la fonction d'assignation par référence;
-   le chaînage des opérations par l'enchaînement des opérateurs `[...]`.

### L'opérateur `[...]`

**La première fonction fondamentale de `data.table` est l'opérateur `[...]` (crochets).** Lorsqu'on les applique à un objet `data.frame` de base, les crochets `df[...]` servent uniquement à sélectionner des lignes ou des colonnes. Dans un `data.table`, les crochets `dt[...]` permettent de faire beaucoup plus de choses (quasiment tout, en pratique). En fait, les instructions à l'intérieur des crochets peuvent être envisagées comme des requêtes `SQL` mises en forme différemment.
...
<!----Encapsuler dans div permet d'éviter que ce soit dans latex ---->

|                        |                                                                                                                  |
|----------------|--------------------------------------------------------|
| \*\*\`Base R\`\*\*     | \`\`\`{r, eval=FALSE} aggregate( dt[dt[["x"]] \> 3]\$y, by = list(dt[dt[["x"]] \> 3]\$z), FUN = sum) \`\`\`      |
| \*\*\`dplyr\`\*\*      | \`\`\`{r, eval=FALSE} dt %\>% dplyr::filter(x \> 3) %\>% dplyr::group_by(z) %\>% dplyr::summarise(sum(y)) \`\`\` |
| \*\*\`data.table\`\*\* | \`\`\`{r, eval=FALSE} dt[x \> 3, sum(y), by = z] \`\`\`                                                          |

</div>

### La fonction d'assignation par référence (ou `:=`) {#assignation-reference}

La seconde fonction fondamentale de `data.table` est la fonction d'*assignation par référence* (`:=`), qui sert à créer ou à modifier des colonnes dans un `data.table`. Cette fonction peut également être appelée comme une fonction: `` `:=`() ``, avec les arguments entre parenthèses. Voici comment on crée une nouvelle colonne dans la table `dt` créée auparavant :
...
| \_Package\_                                  | Code                                                                                                            | Commentaire                                                                          |
|-----------------|-------------------------------|------------------------|
| \`\`\`{asis echo=TRUE} \`dplyr\` \`\`\`      | \`\`\`{r eval=FALSE} bpe_ens_2018 \<- bpe_ens_2018 %\>% dplyr::mutate(nouvelle_colonne = NB_EQUIP \* 10) \`\`\` | Il faut utiliser une assignation (\`\<-\`) pour modifier la table.                   |
| \`\`\`{asis echo=TRUE} \`data.table\` \`\`\` | \`\`\`{r eval=FALSE} bpe_ens_2018_dt[ , nouvelle_colonne := NB_EQUIP \* 10] \`\`\`                              | Il ne faut pas d'assignation pour modifier la table, qui est modifiée par référence. |

### Enchaîner les opérations en `data.table`

Le troisième principe de `data.table` est qu'il est possible d'enchaîner des opérations en chaînant les opérateurs crochet `[...]`. Ce principe est l'équivalent en `data.table` de l'enchaînement des opérations avec le pipe (`%>%`) avec `dplyr`.
...
**L'enchaînement des opérations en `data.table` est puissant, mais peut aboutir à des résultats non désirés si on ne fait pas attention.** Il est fortement conseillé de lire la section [La fonction d'assignation par référence (ou `:=`)](#assignation-reference) avant de poursuivre la lecture de ce paragraphe.
...
::: remarque
Si vous utilisez la notation`` `:=`() ``, alors il faut utiliser uniquement `=` à l'intérieur des parenthèses pour créer ou modifier des variables, et non `:=`. Par exemple, les deux codes suivants sont strictement équivalents, mais la fonction d'assignation par référence est utilisée de façon différente.
...
La fonction `:=` peut être utilisée pour modifier une colonne pour certaines lignes seulement, en fonction d'une condition logique. Cette approche est beaucoup plus rapide qu'un terme `dplyr::if_else()` ou `dplyr::case_when()`. Imaginons qu'on veuille créer une colonne `EQUIP_HORS_CHAUSS` égale au nombre d'équipements (`NB_EQUIP`) sauf pour les lignes correspondantes à des magasins de chaussures (`TYPEQU == "B304"`) où elle vaut `NA`. Dans ce cas, le code `dplyr` serait :
...
-   une [*cheatsheet*](https://github.com/rstudio/cheatsheets/raw/master/datatable.pdf) sur `data.table` (en anglais) ;
